#!/bin/bash
# validate-novel-commit.sh — 在 git commit 时检查格式问题（WARNING only, no BLOCKING）
set -euo pipefail
...
raw = os.environ.get("NOVEL_COMMIT_COMMAND", "")
if not raw:
...
if [ -n "$WARNINGS" ]; then
  echo "=== Novel Commit Warnings (advisory only, not blocking) ==="
  printf '%b\n' "$WARNINGS"
  echo "=== End Warnings ==="
fi
