...
description: |
  ...
  被 novel-review、novel-write（Phase 5）、novel-write（Phase 4）调用。
  不做任何创作判断。
...
评分标准参考 `novel-setup/references/agent-references/quality-checklist.md` 中的五维评分体系（核心一致度、表层重写度、格式一致度、可读性、逻辑连贯），你的检查聚焦于**核心一致度**和**逻辑连贯**两个维度的事实性冲突。
