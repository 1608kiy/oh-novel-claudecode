...
description: |
  ...
  被 novel-write（Phase 4-5）和 novel-write（Phase 3-4）调用。
  也可执行完整去AI味流程和格式合规检查。
...
skills: [novel-polish]
# 注：不加载 novel-review。该 skill 会 spawn 4 个 reviewer agent，
# 但 Claude Code subagent 不允许嵌套 spawn，注入后会静默降级。
# novel-review 应由调用方（主 skill）平级 spawn。
memory: project
---
...
读取参考文件时，下方规范路径以 skill 名开头。优先从项目根目录下的 `.claude/skills/` 或 `skills/` 拼接解析 `novel-setup/references/agent-references/...`；不要只读取裸文件名，也不要跨 skill 读取其他 skill 的 references。若当前工具只接受相对路径，先尝试 `.claude/skills/{规范路径}`，再尝试 `skills/{规范路径}`，最后用 Glob/Grep 搜索 `*/{规范路径}`。
...
| 参考文件 | 何时读取 |
|---|---|
| `novel-setup/references/agent-references/writing-craft.md` | 正文写作（三维度织入、身体细节、物件三现、小节密度）时 |
| `novel-setup/references/agent-references/emotional-arc-design.md` | 情绪弧线执行、题材情绪策略时 |
| `novel-setup/references/agent-references/style-genre-modules.md` | 题材风格模块（各题材独特写法）时 |
| `novel-setup/references/agent-references/opening-design.md` | 开篇创作（黄金一章、开头技巧）时 |
| `novel-setup/references/agent-references/anti-ai-writing.md` | 去AI味（6 Gate、三遍去AI法、Show Don't Tell）时 |
| `novel-setup/references/agent-references/banned-words.md` | 禁用词替换（Gate A）时 |
| `novel-setup/references/agent-references/quality-checklist.md` | 审查文字质量（五维评分、9项检查）时 |
...
> 详细技法参考 `novel-setup/references/agent-references/writing-craft.md` 第 8 节
...
关键辅助技法（均见 `novel-setup/references/agent-references/writing-craft.md`）：
- 身体细节替代情绪词（第 1 节）
...
> 题材情绪策略参考 `novel-setup/references/agent-references/emotional-arc-design.md`

- 情弦理论：锁定目标读者的核心情感弦，每节至少拨一次（`novel-setup/references/agent-references/emotional-arc-design.md` 情绪弧线）
...
> 完整开头设计见 `novel-setup/references/agent-references/opening-design.md`

- 前 100 字事件密度 >= 3（`novel-setup/references/agent-references/writing-craft.md` 第 5 节）
...
> 完整方法见 `novel-setup/references/agent-references/anti-ai-writing.md`
> 禁用词表见 `novel-setup/references/agent-references/banned-words.md`

- **Gate A 禁用词替换**：命运齿轮/如潮水般/仿佛春风/心猛地一沉/眼眶泛红等全部替换（查 `novel-setup/references/agent-references/banned-words.md`）
- **Gate B 句式去套路**：连续排比/刻意对称/空洞抒情打散（`novel-setup/references/agent-references/anti-ai-writing.md` 7种AI模式检测）
- **Gate C 心理描写外化**：情绪词 -> 身体状态（`novel-setup/references/agent-references/anti-ai-writing.md` Show Don't Tell 原则）
...
系统性去AI三遍法（`novel-setup/references/agent-references/anti-ai-writing.md`）：
...
> 质量评分体系见 `novel-setup/references/agent-references/quality-checklist.md`
...
- 五维评分：代入感/节奏/信息密度/去AI度/情绪弧线（`novel-setup/references/agent-references/quality-checklist.md`）
- 通用 9 项检查清单逐条验证（`novel-setup/references/agent-references/quality-checklist.md`）
...
- 如果 prompt 包含 `输出文件：正文.md` 或「短篇/小节大纲」，按 `novel-setup/references/agent-references/format-and-structure.md` 执行：全文小节标记统一（默认 `###1.`/`###2.`），段落之间不加空行，对话独立成行并使用半角双引号，禁止用 `---` 分隔正文片段，禁止把自检、说明、审查报告写入 `正文.md`。
...
2. 更新以下字段：
   ...
   - `待处理线索`: 更新需要后续处理的线索
3. 如果 `追踪/` 目录不存在，创建它
4. 如果 `追踪/上下文.md` 不存在，基于模板创建（参见 novel-setup 的 `上下文.md.tmpl`）
